...
players <- data.table(read_csv("data/players.csv"))

# Each row represents a player in a match thus 10 rows represents a match

# Adding win_percent_legacy to players
playerdata_train <- data.table(left_join(players, legacy_player_ratings[, c("account_id", "win_percent")],
                        by=c("account_id" = "account_id")))
playerdata_train[is.na(win_percent), win_percent:=unidentified_win_percent]

# player_slot 0-4: radiant, 128-132: dire
playerdata_train[, side:= as.factor(if_else(player_slot < 100, true = "RADIANT", false = "DIRE"))]

teamstat_train <- playerdata_train%>% group_by(match_id, side) %>% summarise(win_percent_legacy=mean(win_percent))
teamstat_train <- left_join(teamstat_train[teamstat_train$side == "RADIANT", c("match_id", "win_percent_legacy")],
                teamstat_train[teamstat_train$side == "DIRE", c("match_id", "win_percent_legacy")],
                by="match_id", suffix=c("_RADIANT", "_DIRE"))

# Test data: lets do the same
players <- data.table(read_csv("data/test_player.csv"))
playerdata_test <- data.table(left_join(players, legacy_player_ratings[, c("account_id", "win_percent")],
                       by=c("account_id" = "account_id")))
playerdata_test[is.na(win_percent), win_percent:=unidentified_win_percent]
playerdata_test[, side:= as.factor(if_else(player_slot < 100, true = "RADIANT", false = "DIRE"))]

teamstat_test <- playerdata_test%>% group_by(match_id, side) %>% summarise(win_percent_legacy=mean(win_percent))
teamstat_test <- left_join(teamstat_test[teamstat_test$side == "RADIANT", c("match_id", "win_percent_legacy")],
                teamstat_test[teamstat_test$side == "DIRE", c("match_id", "win_percent_legacy")],
                by="match_id", suffix=c("_RADIANT", "_DIRE"))


train <- left_join(ranked_match_outcome_train, teamstat_train, by="match_id")
test <- left_join(ranked_match_outcome_test, teamstat_test, by="match_id")

# rm(playerdata, teamstat, train, test)
